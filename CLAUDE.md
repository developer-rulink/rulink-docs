...
| `system/` | Системные настройки | rulink.io/legal |
| `examples/` | Служебный проект — примеры разметки | rulink.io/support/auth |
...
- **Theme:** Material, dark mode enabled, language `ru`
- **Logo/Favicon:** `assets/favicon.svg`
- **Copyright:** `2025/2026 © ООО Облакотех`
...
When adding a new service, create a directory with a `docs/` subdirectory containing at least `index.md`, and mirror the `mkdocs.yml` structure from an existing service.

## Adding New Documentation

- New service: add a folder with `docs/index.md` and `mkdocs.yml` matching existing conventions.
- New page in existing service: add `.md` file to `docs/`, then register it under `nav:` in that service's `mkdocs.yml`.
- Images shared across services: place in `images/` directory and reference via `https://rulink.io/images/{filename}`.
