...
Подробная инструкция для настройки wildcard-сертификатов: [xxx](xxx)
Подробная инструкция для создания и настройки самоподписанных сертификатов: [xxx](xxx)

### Шаг 5. Настройка шлюза приложений nginx

Подробная инструкция по настройке публикации приложения: [xxx](xxx)

Необходимо настроить публикацию сервиса Identity. Используйте `доменное имя` и `ssl.conf`, предназначенные для сервиса.
Если у вас уже есть работающий сервис Identity — можно использовать его. В этом случае пропустите публикацию Identity из текущей поставки.
...
### Шаг 7. Проверьте доступность сервисов
