...
TLS-сертификат обеспечивает шифрование данных между браузером пользователя и сервером. Без него браузеры помечают сайт как небезопасный, а современные API недоступны без HTTPS. Для корректной работы сервисов Облакотех SSL-сертификат обязателен.
...
Полная статья как опубликавать сервис в разделе [nginx](nginx.md)
